...
| Server-side NTS-NTP authentication | **complete port of `nts_ntp_server.c`** (all 4 functions): parse request EFs, decode cookie → keys, verify/decrypt the authenticator, build the authenticated response; composes ntp_ext + nts_ntp_auth + real SIV; vs the REAL compiled nts_ntp_server.c (byte-identical response) + a full round-trip |
