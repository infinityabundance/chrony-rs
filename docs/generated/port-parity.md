...
- **chrony-rs native inventory (`syn` AST):** 1412 named functions + 297 closures across 95 `.rs` files. Extracted from the real AST, not doxygen — see the limitation notice in `docs/port-parity.md`.
...
| `ntp_core.c` | 69 | 18.8% | NTP protocol engine: poll, process-response, offset/delay (NCR_*) | `ntp/measurements.rs`<br>`ntp/packet.rs`<br>`ntp/poll.rs`<br>`ntp/parse.rs` | ◑ partial |
