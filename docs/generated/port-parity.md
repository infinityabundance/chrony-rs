...
- **chrony-rs native inventory (`syn` AST):** 1306 named functions + 254 closures across 85 `.rs` files. Extracted from the real AST, not doxygen — see the limitation notice in `docs/port-parity.md`.
...
| `sources.c` | 48 | 41.7% | source reachability + selection (SRC_*) | `sources/registry.rs`<br>`sources/source.rs`<br>`sources/reachability.rs`<br>`sources/selection.rs` | ◑ partial |
