...
- **chrony-rs native inventory (`syn` AST):** 1408 named functions + 294 closures across 93 `.rs` files. Extracted from the real AST, not doxygen — see the limitation notice in `docs/port-parity.md`.
...
| `ntp_core.c` | 69 | 17.4% | NTP protocol engine: poll, process-response, offset/delay (NCR_*) | `ntp/measurements.rs`<br>`ntp/packet.rs`<br>`ntp/poll.rs`<br>`ntp/parse.rs` | ◑ partial |
