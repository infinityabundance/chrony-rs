...
- **Files with any chrony-rs counterpart:** 26 / 70 (15 full, 8 partial, 3 scaffold); **44** have none.
- **Files fully ported:** 15 / 70 — every function in the unit has a court-backed counterpart (dependency-free TUs first). chrony-rs remains an early-stage forensic reconstruction; this number is stated, not hidden.
- **Loose upper bound on function coverage:** files with a counterpart contain 689 / 1373 C functions (50.2%). This is an *upper bound only* — a file marked partial ports a fraction of its functions, so true coverage is well below this. chrony-rs ports behavior under court, not functions 1:1.

- **chrony-rs native inventory (`syn` AST):** 449 named functions + 67 closures across 41 `.rs` files. Extracted from the real AST, not doxygen — see the limitation notice in `docs/port-parity.md`.
