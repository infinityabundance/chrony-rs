...
- **chrony-rs native inventory (`syn` AST):** 1364 named functions + 268 closures across 87 `.rs` files. Extracted from the real AST, not doxygen — see the limitation notice in `docs/port-parity.md`.
...
| `sources.c` | 48 | 77.1% | source reachability + selection (SRC_*) | `sources/registry.rs`<br>`sources/combine.rs`<br>`sources/source.rs`<br>`sources/reachability.rs`<br>`sources/selection.rs` | ◑ partial |
