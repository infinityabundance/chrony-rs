...
fn dump_save_format_and_round_trip_match_real_c() {
    ...
    // Full byte-parity: the SST sample body (the rest of the dump) must equal the real C's
    // SST_SaveToFile output for the same samples -- pinning the %.6e exponent format through
    // save_source (the SST portion was previously untested here; only the header was checked).
    let want_body = find("SSTBODY").split_once(" |").unwrap().1.strip_suffix('|').unwrap().replace("\\n", "\n");
    let got_body: String = lines.map(|l| format!("{l}\n")).collect();
    assert_eq!(got_body, want_body, "dump SST sample body (byte-exact)");
