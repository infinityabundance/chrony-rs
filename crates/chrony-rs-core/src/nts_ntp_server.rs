...
impl NtsServer {
    ...
    pub fn check_request_auth(
        ...
        if !self.sivs[i].as_mut().unwrap().set_key(&context.c2s.key) {
            return (false, 0);
        }

        let mut plaintext = vec![0u8; NTP_MAX_EXTENSIONS_LENGTH as usize];
        let Some(plaintext_length) =
            decrypt_auth_ef(packet, info, self.sivs[i].as_mut().unwrap(), auth_start, &mut plaintext)
        else {
            return (false, NTP_KOD_NTS_NAK);
        };
        ...
        if !self.sivs[i].as_mut().unwrap().set_key(&context.s2c.key) {
            return (false, 0);
        }
    ...
    pub fn generate_response_auth(
        ...
        assert_eq!(self.req_tx, request.bytes()[40..48], "response to a different request");
        ...
        while parsed < req_info.length {
            ...
            if pf.field_type == NTP_EF_NTS_UNIQUE_IDENTIFIER {
                let body =
                    request.bytes()[pf.body_offset..pf.body_offset + pf.body_length as usize].to_vec();
        ...
        let siv = self.sivs[self.siv_index as usize].as_mut().unwrap();
        generate_auth_ef(
            response,
            res_info,
            siv,
