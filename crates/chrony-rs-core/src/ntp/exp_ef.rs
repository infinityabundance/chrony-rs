...
//! seeded from the system CSPRNG). The random draw is a host-boundary concern injected
//! elsewhere in the reconstruction, but the fuzz's deterministic placement is the ported
//! [`crate::util::get_ntp64_fuzz`]; these builders produce the **unfuzzed** body and the
//! caller XORs in fuzz if required. (The oracle zeroes the fuzz to match.)
//!
//! # Oracle
//!
