    ...
    for l in v.lines().map(str::trim) {
        if l.starts_with("RK_") {
            ...
            let want = match field(l, "action") {
                ...
                "ProcBroadcast" => RxKnownAction::ProcessBroadcast,
                "ProcSymPassive" => RxKnownAction::ProcessSymmetricPassive,
...
fn known_pairings() {
    ...
    assert_eq!(
        classify_rx_known(MODE_PASSIVE, MODE_ACTIVE),
        ProcessResponse
    );
    // Any client request is routed to the unknown handler.
    assert_eq!(
        classify_rx_known(MODE_CLIENT, MODE_CLIENT),
        ProcessAsUnknown
    );
    // Active from an unconfigured peer triggers symmetric passive (ephemeral).
    assert_eq!(
        classify_rx_known(MODE_ACTIVE, MODE_CLIENT),
        ProcessSymmetricPassive
    );
    // Broadcast from a configured broadcast source is processed.
    assert_eq!(
        classify_rx_known(MODE_BROADCAST, MODE_CLIENT),
        ProcessBroadcast
    );
}
...
fn unknown_reply_modes() {
    assert_eq!(
        classify_rx_unknown(MODE_CLIENT, 4, 50000),
        Some(MODE_SERVER)
    );
    ...
    assert_eq!(
        classify_rx_unknown(MODE_UNDEFINED, 1, 50000),
        Some(MODE_SERVER)
    );
