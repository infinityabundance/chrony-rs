...
/// `sock_initialise`: open the SOCK refclock datagram socket and register
/// the file handler. Host boundary (socket open + scheduler registration).
pub fn sock_initialise<F: FnOnce()>(init: F) {
    init();
}

/// `sock_finalise`: close the SOCK refclock socket. Host boundary.
pub fn sock_finalise<F: FnOnce()>(finalise: F) {
    finalise();
}
