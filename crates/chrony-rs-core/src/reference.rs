...
impl Timespec {
    ...
    pub fn add_double(self, increment: f64) -> Timespec {
        ...
        Timespec {
            sec: sec + nsec.div_euclid(1_000_000_000),
            nsec: nsec.rem_euclid(1_000_000_000) as i32,
        }
    }
    /// `UTI_IsZeroTimespec`.
...
pub const NTP_REFID_LOCAL: u32 = 0x7F7F_0101;
...
impl Default for RefConfig {
    fn default() -> RefConfig {
        ...
        RefConfig {
            ...
            make_step_limit: 3, // match chrony upstream default
...
impl Reference {
    ...
    pub fn set_reference(
        ...
        let update_interval = if self.last_ref_update != 0.0 {
            mono_now - self.last_ref_update
        } else {
            0.0
        };

        let residual_frequency;
        (frequency, skew, residual_frequency) = self.get_clock_estimates(manual, frequency, skew);
        ...
        if step_offset != 0.0 && host.apply_step_offset(step_offset) {
            host.log_message(&format!(
                "System clock was stepped by {:.6} seconds",
                -step_offset
            ));
        }
    ...
    pub fn adjust_reference(&mut self, host: &mut dyn RefHost, offset: f64, frequency: f64) -> i32 {
        ...
        let adj_corr_rate =
            self.get_correction_rate(offset.abs(), mono_now - self.last_ref_adjustment);
        let ref_corr_rate = self
            .get_correction_rate(self.our_offset_sd, self.last_ref_update_interval)
    ...
    pub fn get_reference_params(
        ...
        {
            let leap_status = if !self.leap_in_progress {
    ...
    /// Return the reference ID set by the last call to set_reference.
    pub fn ref_id(&self) -> u32 {
        self.our_ref_id
    }

    /// Return the stratum set by the last call to set_reference.
    pub fn stratum(&self) -> i32 {
        self.our_stratum
    }

    /// Return the current (last applied) offset.
    ...
    /// Return the residual frequency in ppm.
    pub fn freq_ppm(&self) -> f64 {
        1.0e6 * self.our_residual_freq
    }

    /// Return the skew in ppm.
