...
use crate::nts_ntp_server::{
    NkeContext, NkeCookie, NkeKey, NTP_EF_NTS_COOKIE, NTP_EF_NTS_COOKIE_PLACEHOLDER,
...
// ---------------------------------------------------------------------------
// Production NTS-KE client — bridges the TlsSession trait to the NkeClient
// trait that NtsClient expects. This is the production wiring of
// KeHandshakeResult → NkeData / cookie codec → NTS authenticator.
// ---------------------------------------------------------------------------

use crate::nts_tls::{perform_ke_exchange, TlsSession};

/// A production implementation of [`NkeClient`] that performs a real TLS 1.3
/// handshake via the injected [`TlsSession`], calls `perform_ke_exchange()`,
/// and maps the [`KeHandshakeResult`] into the [`NkeData`] that [`NtsClient`]
/// consumes. This closes the gap between the ported NTS-KE TLS layer and the
/// ported NTS-NTP client auth pipeline.
pub struct RealNkeClient {
...
impl RealNkeClient {
    /// Create a new production NKE client backed by `tls`. The `host`/`port`
    /// are the NTS-KE server endpoint; the TLS SNI and cert verification use
    /// the hostname from [`NkeClient::create`].
    pub fn new(tls: Box<dyn TlsSession>, host: String, port: u16) -> Self {
        RealNkeClient {
            tls: Some(tls),
            host, port,
            name: String::new(),
...
impl NkeClient for RealNkeClient {
    fn create(&mut self, _address: &NtpAddress, name: &str, _cert_set: u32) {
    ...
    fn start(&mut self) -> bool {
        ...
        // Perform the NTS-KE exchange: TLS handshake → record exchange → key export
        match perform_ke_exchange(tls.as_mut(), &self.host, self.port) {
            Ok(result) => {
                // Map KeHandshakeResult → NkeData
                let algorithm = if result.key_length == 32 {
                    SivAlgorithm::AesSivCmac256
                } else {
                    SivAlgorithm::Aes128GcmSiv
                };
                self.data = Some(NkeData {
                    context: NkeContext {
                        algorithm,
                        c2s: NkeKey { key: result.c2s_key },
                        s2c: NkeKey { key: result.s2c_key },
                    },
                    cookies: result.cookies.into_iter()
                        .map(|bytes| NkeCookie { bytes })
                        .collect(),
                    ntp_address: NtpAddress { ip: None, port: 0 },
                });
                self.tls = Some(tls);
                true
            }
    ...
    fn is_active(&mut self) -> bool {
        false // blocking implementation
    }

    fn get_nts_data(&mut self, _max_cookies: usize) -> Option<NkeData> {
        self.data.take()
    }
