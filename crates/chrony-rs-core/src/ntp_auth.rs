...
fn generate_symmetric_auth(
    ...
    let mut max_auth_len = (if info.version == 4 {
...
impl fmt::Debug for NauInstance {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NauInstance")
            .field("mode", &self.mode)
            .field("key_id", &self.key_id)
            .field("nts", &self.nts.as_ref().map(|_| "<NtsClient>"))
            .finish()
    }
}

impl NauInstance {
    /// The configured key id for this auth instance.
        ...
        match self.mode {
            ...
            NtpAuthMode::Nts => {
                if !self
                    ...
                    .as_mut()
                    .unwrap()
                    .generate_request_auth(request, info)
                {
                    return false;
                }
            }
    ...
    pub fn check_response_auth(
        ...
        match self.mode {
            ...
            NtpAuthMode::Nts => self
                .nts
                .as_mut()
                .unwrap()
                .check_response_auth(response, info),
            _ => false,
        }
    }
...
pub fn generate_response_auth(
    ...
    match NtpAuthMode::from_i32(request_info.auth_mode) {
        ...
        NtpAuthMode::Nts => {
            if !nts_server.generate_response_auth(
                request,
                request_info,
                response,
                response_info,
