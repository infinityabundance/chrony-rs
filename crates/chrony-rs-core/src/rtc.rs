...
impl RtcManager {
    ...
    pub fn finalise(&mut self) {
        if self.driver_initialised {
            self.driver.as_mut().unwrap().finalise();
        }
    }
    ...
    pub fn time_init(&mut self, after_hook: Box<dyn FnMut()>) {
        if self.driver_initialised && self.driver_preinit_ok {
            self.driver.as_mut().unwrap().time_init(after_hook);
        } else {
    ...
    pub fn start_measurements(&mut self) {
        if self.driver_initialised {
            self.driver.as_mut().unwrap().start_measurements();
        }
    }
    ...
    pub fn write_parameters(&mut self) -> i32 {
        if self.driver_initialised {
            self.driver.as_mut().unwrap().write_parameters()
        } else {
            RTC_ST_NODRV
        }
    }
    ...
    pub fn get_report(&mut self) -> Option<RtcReport> {
        if self.driver_initialised {
            self.driver.as_mut().unwrap().get_report()
        } else {
            None
        }
    }
    ...
    pub fn trim(&mut self) -> i32 {
        if self.driver_initialised {
            self.driver.as_mut().unwrap().trim()
        } else {
            0
        }
    }
